...
const Sidebar = () => {
  ...
  return (
    <div className={` ${open ? 'w-60' : 'w-18'} bg-[#111729] h-full relative duration-300`}>
      <FaCircleArrowLeft className={`absolute cursor-pointer -right-2 border border-black top-28 rounded-full text-3xl ${!open && 'rotate-180'} text-white`} onClick={toggleSideBar} />
      <div className="flex h-screen items-center flex-col justify-around">
        ...
        <div className="flex-col flex items-center justify-center gap-10 w-full">
          <div className="flex items-center justify-center gap-5">
            <Image src="/user.png" width={45} height={45} alt="Picture of the author" className="bg-white rounded-full" />{' '}
            <span className={`${!open && 'hidden'} origin-left duration-200 text-white`}>Aliam Colter</span>
          </div>

          <div className="flex items-center justify-around w-full gap-5">
            <FaArrowRightFromBracket size={20} className="text-[#CCCCCC]" />
            <span className={`${!open && 'hidden'} origin-left duration-200 text-white`}>Logout</span>
          </div>
        </div>
      </div>
    </div>
